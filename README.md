...
> [!IMPORTANT]
> v6.0 正在按 AI-Native PRD 进行底座迁移。当前仓库根目录的 Next.js 应用是可运行的 v5 迁移源，不是 v6 最终技术底座；v6 以固定提交的 EvalScope 为唯一执行与 Web 基座，旧代码只按清单迁移可复用能力。统一入口见 [v6.0 重构基线](docs/v6/README.md)。
...
当前仓库已冻结 v5 的横向扩功能开发，进入 v6.0 底座迁移阶段。每个 PR 必须覆盖功能验收、异常路径、真实源文件测试、文档状态、安全检查、回滚说明以及截图或 Trace 证据；没有代码和测试证据的能力不得写成“已完成”。

- v6.0 统一入口：[`docs/v6/README.md`](docs/v6/README.md)
- v6.0 底座核验：[`docs/v6/底座能力核验表.md`](docs/v6/底座能力核验表.md)
- v6.0 分阶段验收：[`docs/v6/分阶段验收用例.md`](docs/v6/分阶段验收用例.md)
