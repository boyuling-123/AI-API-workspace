...
test("Chinese lab records recovery, exports actual spans, and stays offline", async ({ page, safePage }) => {
  ...
  await page.getByText("查看原始步骤树", { exact: true }).click();
  await expect(page.getByRole("tree").getByText("库存查询：首次失败（故障注入）", { exact: true })).toBeVisible();
  await expect(page.getByRole("tree").getByText("库存查询：重试恢复（本地桩）", { exact: true })).toBeVisible();
...
test("lab is reachable from overview and fits a narrow viewport", async ({ page, safePage }) => {
  ...
  await page.getByText("查看原始步骤树", { exact: true }).click();
  await expect(page.getByRole("tree").getByText("规则检查 Agent（模拟）", { exact: true })).toBeVisible();
