...
| 当前存储 | `src/services/db.ts` | 隔离 ProjectRepository，仍用 Dexie | 不新建后端、不升 Schema、不清理用户数据 |

现有单测和 E2E 可以作为迁移验收资产，但旧通过记录不能替代新接线后的测试。旧 `listCompatibleProjects` 存在删除不兼容记录的行为，任何新迁移必须先提供保留/隔离策略；本次调研没有打开用户 IndexedDB。
...
`package-lock.json` 保留依赖精确版本与完整性摘要；原 LICENSE 随 npm 包分发。服务不使用网络 Exporter。新路由不依赖现有项目数据库，SDK 在主动点击后动态加载；Ant Design 的依赖位于该页面块而非强塞旧首页。
