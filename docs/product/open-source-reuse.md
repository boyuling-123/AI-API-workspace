...
- 可迁移先落在带 schemaVersion/source/版本信息的 JSON 与原始证据引用。F-OBS-004 已完成本平台两种 Mock 观测文件确认式回读的本地验收（[PR #53](https://github.com/boyuling-123/AI-API-workspace/pull/53)，最终CI待验），强制保留“外部文件，来源未认证”；不支持任意OTLP/业务数据或全项目迁移，不称整个平台一键迁移已完成。
