...
| JUDGE-006 | 高风险与高频分歧 Case 可人工复核 | 已实现 | `calibrationReview.ts` 以固定规则生成可解释风险与稳定队列；`CalibrationReviewQueue.tsx` 支持领取、复核结论、必填说明和只追加审计；`Project.calibrationReviewEvents` 刷新持久化且不覆盖原始校准证据 | `calibrationReview.test.ts` 6 项覆盖错误、漏判、分歧、低置信度、重复频次、脱敏、异常与篡改；`calibration-review-queue.spec.ts` 覆盖领取、改判、审计、刷新零新增调用和 WCAG；最新基线独立环境 quality 与全量 31 项 Playwright 通过 | 风险规则可解释，Case 可领取、复核和留痕 | 报告复核 | 待关联 |
...
| REPORT-005 | 筛选低分、分歧、高风险和失败 Case | 部分实现 | 校准复核队列可按严重/高风险、分歧与漏判、调用失败、低置信度、处理状态和关键词筛选并一键清除；通用评价低分筛选、组合导出尚未实现 | `calibration-review-queue.spec.ts` 验证失败筛选空态与清除、风险依据和刷新状态；最新基线独立环境全量 31 项 Playwright 通过 | 四类筛选可组合、清除并导出 | 报告复核 | 待关联 |
| REPORT-006 | 人工修改评分、标记 Bad Case 和补充意见 | 部分实现 | 校准 Case 支持确认 Judge、人工改判 pass/fail 或待后续处理，强制填写说明并保存修改人、时间与原始标签；通用 EvaluationRecord 分数覆盖和 Bad Case 标记尚未实现 | 真实源码测试验证只追加、跨人阻断、空说明和原始标签不变；Mock E2E 验证领取、改判、审计和刷新持久化 | 人工覆盖保留原 AI 分数、修改人和理由 | 报告复核 | 待关联 |
