...
| REPORT-005 | 筛选低分、分歧、高风险和失败 Case | 部分实现 | 校准复核队列可按严重/高风险、分歧与漏判、调用失败、低置信度、处理状态和关键词筛选并一键清除；通用评价低分筛选、组合导出尚未实现 | `calibration-review-queue.spec.ts` 验证失败筛选空态与清除、风险依据和刷新状态；最新基线独立环境全量 31 项 Playwright 通过 | 四类筛选可组合、清除并导出 | 报告复核 | [PR #35](https://github.com/boyuling-123/AI-API-workspace/pull/35) |
