...
| JUDGE-005 | 支持单 Judge、多 Judge 和分歧仲裁 | 部分实现 | `multiJudgeCalibration.ts` 定义 2-5 Judge 独立投票、逐 Judge 指标、保守多数票/全票通过策略与完整证据复算；`multiJudgeCalibrationClient.ts` 按 Case×Judge 精确矩阵使用全局并发池；`evaluatorRelease.ts` 在 Active 前复算并冻结 Judge 集合与策略；当前页面仍只支持单 Judge | `multiJudgeCalibration.test.ts`、`multiJudgeCalibrationClient.test.ts` 与 `evaluatorRelease.test.ts` 覆盖投票、平票、失败隔离、非法策略、残缺/篡改证据、精确请求、全局并发、真值隔离和发布门禁；本地、独立干净环境与 workflow run `33297825582` 两道 CI 全部通过 | 页面可选择多个 Judge、预览 Case×Judge 费用、确认后执行，并下钻原始票、逐 Judge 指标和仲裁结果；通过视觉验收 | Judge 校准 | [#32](https://github.com/boyuling-123/AI-API-workspace/pull/32) |
