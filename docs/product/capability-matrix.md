...
| REPORT-004 | 查看 Judge 引用证据 | 已实现 | `evaluateService.ts` 要求每个目标的每个维度返回 1–3 条结构化文字或图片证据，服务端核验来源、目标、原文与图片序号并自行计算引用位置；`EvaluationEvidenceList.tsx` 在即时结果与历史详情展示引用，`excel.ts` 导出独立证据列；旧记录明确显示未保存结构化证据 | 12 项真实源码单测覆盖精确位置、标准答案、输入/输出图片、客户端组包、未知目标、伪造原文、缺失与重复证据、零调用前置阻断及 Excel；Mock E2E 覆盖即时展开、历史持久化、xlsx 回读、390px、WCAG 和零新增调用；本地 quality 与全量 35 项 Playwright 通过 | 每条结论包含可定位到输入或输出的证据 | 报告复核 | 待关联 |
