...
| REPORT-001 | 输出综合排行榜和单维度排行榜 | 已实现 | `evaluationLeaderboard.ts` 仅从历史原始分数计算确定性综合榜/单维度榜，`EvaluationLeaderboard.tsx` 展示正式名次、综合分、覆盖率、否决次数与维度均分 | 5 项真实源码单测覆盖权重归一、缺失值不补零、并列排名与旧记录；`evaluation-leaderboard.spec.ts` 验证综合榜、单维度反转、零新增调用、刷新和 WCAG；本地 quality 与全量 32 项 Playwright 通过 | 排名口径可解释并可下钻原始分数 | 报告复核 | 待关联 |
