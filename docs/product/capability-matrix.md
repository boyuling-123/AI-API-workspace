...
| REPORT-005 | 筛选低分、分歧、高风险和失败 Case | 已实现 | `evaluationCaseFilter.ts` 以历史加权分、否决结果和来源运行状态确定性识别四类 Case；历史详情支持可调阈值、任一/全部组合、清除、依据标签和筛选子集 Excel，校准复核队列继续覆盖 Judge 专项风险 | 5 项真实源码单测覆盖四类隔离、组合语义、阈值、旧记录补算、缺失值与稳定导出；Mock E2E 验证 5 Case、xlsx 回读、空态、刷新零调用、390px 和 WCAG；本地及独立环境 quality 与全量 33 项 Playwright 通过，CI 待验收 | 四类筛选可组合、清除并导出 | 报告复核 | PR 07C 待创建 |
