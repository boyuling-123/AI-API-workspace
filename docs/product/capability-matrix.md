...
| TASK-005 | 仅重跑失败项、指定 Case、新模型或新维度 | 部分实现 | `rerunPlan.ts`、`batchCheckpoint.ts`、`runService.ts`、`RerunDialog.tsx` 支持失败组合、指定 Case 和新增目标的稀疏新任务；新增目标仅增量调用并显式复用历史结果；已有结果可新增维度评价，但该路径尚缺独立预览与追溯验收 | `rerunPlan.test.ts`、`batchCheckpoint.test.ts`、`runService.test.ts` 覆盖真实源码；`selective-rerun.spec.ts` Mock 验证三类精确请求、零误调用、历史复用、来源追溯与 WCAG；PR #15 与 PR #16 两道 CI 通过 | 四种重跑范围均可预览、确认和追溯 | 任务控制 | [#15](https://github.com/boyuling-123/AI-API-workspace/pull/15)；[#16](https://github.com/boyuling-123/AI-API-workspace/pull/16) |
