...
| REPORT-001 | 输出综合排行榜和单维度排行榜 | 已实现 | `evaluationLeaderboard.ts` 仅从历史原始分数计算确定性综合榜/单维度榜，`EvaluationLeaderboard.tsx` 展示正式名次、综合分、覆盖率、否决次数与维度均分 | 5 项真实源码单测覆盖权重归一、缺失值不补零、并列排名与旧记录；`evaluation-leaderboard.spec.ts` 验证综合榜、单维度反转、零新增调用、刷新和 WCAG；本地与独立干净环境的 quality、全量 32 项 Playwright 均通过 | 排名口径可解释并可下钻原始分数 | 报告复核 | 待关联 |
| REPORT-002 | 勾选关注维度后动态重新排名 | 已实现 | 排行榜支持勾选任意关注维度、仅看单维度和恢复全选；平台按所选维度的原权重重新归一，计算过程不写回 `EvaluationRecord` | 单测验证单维度重排前后原记录字节级不变；Mock E2E 验证准确性单维度使名次反转且 API 调用数不增加，390px 页面无横向溢出 | 维度选择即时更新排名且不篡改原始结果 | 报告复核 | 待关联 |
| REPORT-003 | 查看模型、Case、维度评分和理由 | 已实现 | `EvalHistoryPanel.tsx` 保留输入、输出、模型、各维度分数、hover 理由、策略和点评明细；排行榜提供原始 Case 锚点 | `evaluation-leaderboard.spec.ts` 从排行榜下钻并验证 Case 输入和原始维度理由；本地全量 32 项 Playwright 通过 | 任一评分均能下钻到输入、输出、维度和理由 | 报告复核 | 待关联 |
