...
| TASK-005 | 仅重跑失败项、指定 Case、新模型或新维度 | 部分实现 | `rerunPlan.ts`、`batchCheckpoint.ts`、`runService.ts`、`RerunDialog.tsx` 已支持失败组合和指定 Case 的稀疏新任务；已有结果可新增维度评价；新增模型重跑仍缺失 | `rerunPlan.test.ts`、`batchCheckpoint.test.ts`、`runService.test.ts` 覆盖真实源码；`selective-rerun.spec.ts` Mock 验证精确请求、零误调用、来源追溯与 WCAG；PR #15 两道 CI 通过 | 四种重跑范围均可预览、确认和追溯 | 任务控制 | [#15](https://github.com/boyuling-123/AI-API-workspace/pull/15) |
...
- TASK-005 已通过 PR #15 完成失败项和指定 Case 两种稀疏重跑，并具备来源追溯、干净环境和 CI 证据；新增模型重跑尚未实现，因此在四种范围全部验收前继续保持“部分实现”。
