...
| JUDGE-005 | 支持单 Judge、多 Judge 和分歧仲裁 | 已实现 | `multiJudgeCalibration.ts` 与客户端按 `2-5` Judge 建立独立调用矩阵、确定性仲裁和完整证据复算；`JudgeCalibrationPanel.tsx` 新增显式多选、两种策略、Case×Judge 精确确认、逐 Judge 指标和原始票下钻；`evaluatorRelease.ts` 在 Active 前复算并冻结 Judge 集合与策略；旧单 Judge 历史兼容 | 核心单测覆盖投票、平票、失败隔离、非法策略、证据篡改、精确请求、全局并发和发布门禁；`multi-judge-calibration.spec.ts` 以 Mock 验证 `2×3=6`、取消零调用、真值隔离、刷新持久化和三处 Axe，既有单 Judge 回归通过；本地 146 单测、2 压测、20 路由构建和全量 28 E2E 通过，视觉证据见 `docs/evidence/pr-06h` | 页面可选择多个 Judge、预览 Case×Judge 费用、确认后执行，并下钻原始票、逐 Judge 指标和仲裁结果；独立干净环境与 GitHub CI 通过后升级“已验证” | Judge 校准 | [#32](https://github.com/boyuling-123/AI-API-workspace/pull/32) + PR 06H |
...
- JUDGE-005 已由 PR 06G 完成独立投票矩阵、确定性仲裁、失败隔离和发布前证据复算，PR 06H 本地完成多选、精确费用确认、逐 Judge 指标和原始票下钻；本地全量门禁与视觉验收通过，状态为“已实现”，待独立干净环境和 GitHub CI 后升级“已验证”。
