...
| DOC-003 | 准确说明维度生成上下文与 OpenJudge 状态 | 已验证 | `EvaluationPanel.tsx` 明示 OpenJudge 与 Iterative Rubrics Generator 未接入；`dimensionGeneration.ts`、`dimensionHumanFeedback.ts` 与 `genDimensionsService.ts` 使用目标、场景、任务类型、内部预设、有界样本、硬规则、显式 Bad Case 和人工反馈 | `dimensionGeneration.test.ts` 覆盖真实请求、Prompt、边界与脱敏；`dimension-generation.spec.ts`、`dimension-rules.spec.ts` 与 `dimension-human-feedback.spec.ts` 覆盖页面口径、Mock 精确请求与零自动评价；PR #18、#19、#20 两道 CI 通过 | 文档、页面、Schema 与真实模型输入一致；准确区分当前人工反馈上下文、未接入的 OpenJudge 和未实现的完整 Rubrics Generator | PR 01 / 04A / 04B / 04C | [#9](https://github.com/boyuling-123/AI-API-workspace/pull/9)；[#18](https://github.com/boyuling-123/AI-API-workspace/pull/18)；[#19](https://github.com/boyuling-123/AI-API-workspace/pull/19)；[#20](https://github.com/boyuling-123/AI-API-workspace/pull/20) |
