...
[N6最终文档交付](../execution/N6-CLOSEOUT.md)同样不新增能力或改变状态。下一业务阶段仍以新PRD差距审计为准，本夜不再继续开发。
...
新增独立能力 F-STORE-001：已验证（不兼容本地项目保留，限定范围）。[需求与验收](../features/F-STORE-001/spec.md)、代码 `src/services/db.ts`、`src/hooks/useProject.ts`、`src/components/AppShell.tsx`、[9 项新增单测 / 4 项新增 E2E、截图与最终 CI](../evidence/pr-legacy-project-preservation/README.md)。本地门禁与最终 head CI 通过，[PR #47](https://github.com/boyuling-123/AI-API-workspace/pull/47) 已合并；不是迁移、备份或新的存储后端，不改变旧 75 项统计基数。
...
| DOC-004 | 七个 Skill、MCP、Judge 校准和排行榜使用真实状态 | 已实现 | 本矩阵相关条目；`PlatformOverview.tsx` 把六条产品链路分别标为已验证、已实现、部分实现或 Demo，并把 20GB 级后端存储明确标为设计中 | `workspace.spec.ts` 验证总览真实状态、深链、零 API 调用、移动端无横向溢出与 WCAG；PR #34 workflow run `33298811346` 两个 Job 通过；待补七个 Skill 和排行榜的完整口径检查 | 所有未落地能力均为设计中、Demo 或部分实现 | PR 01 / 平台总览短 PR | [#9](https://github.com/boyuling-123/AI-API-workspace/pull/9)；[#34](https://github.com/boyuling-123/AI-API-workspace/pull/34) |
...
| PROMPT-007 | Prompt 或维度变化后重新校准 Judge | 已验证 | `judgeCalibrationRerun.ts` 对不可变 Evaluator 版本执行定义做确定性变更检测；`JudgeCalibrationPanel.tsx` 自动生成确认式重跑计划并关联基线；`evaluatorRelease.ts` 与 `EvaluatorReleaseGate.tsx` 在失败时阻断 Active 发布 | 重跑规划与发布门禁真实源码单测；`judge-calibration-rerun.spec.ts` 和 `evaluator-release-gate.spec.ts` 覆盖 v1→v2、失败阻断、通过后发布、刷新持久化与零发布调用；本地、独立干净工作树与 workflow run `33297002754` 两道 CI 全部通过 | 变更触发黄金集校准，失败时无法成为 Active，达标后仍需发布人和二次确认 | Judge 校准 | [#30](https://github.com/boyuling-123/AI-API-workspace/pull/30)；[#31](https://github.com/boyuling-123/AI-API-workspace/pull/31) |
...
| JUDGE-004 | 计算 Judge 与人工判断的一致性、准确率和漏判 | 已验证 | PR #28 的 `judgeCalibrationService.ts`、`/api/judge-calibration` 与 `judgeCalibration.ts` 提供真值隔离和确定性指标；PR #29 的 `judgeCalibrationClient.ts` 与 `JudgeCalibrationPanel.tsx` 提供确认式受控运行、历史持久化、指标卡、混淆矩阵和分歧/失败下钻 | 13 项指标/服务/路由/客户端真实源码测试；`judge-calibration.spec.ts` 覆盖取消零调用、精确请求、部分失败、刷新持久化、WCAG 与 100 Case 数字确认；本地和独立干净工作树的 quality、25 项 Playwright 及 workflow run `33295087238` 两道 CI 全部通过 | 输出明确统计指标和样本下钻 | Judge 校准 | [#28](https://github.com/boyuling-123/AI-API-workspace/pull/28)，[#29](https://github.com/boyuling-123/AI-API-workspace/pull/29) |
| JUDGE-005 | 支持单 Judge、多 Judge 和分歧仲裁 | 已验证 | `multiJudgeCalibration.ts` 与客户端按 `2-5` Judge 建立独立调用矩阵、确定性仲裁和完整证据复算；`JudgeCalibrationPanel.tsx` 新增显式多选、两种策略、Case×Judge 精确确认、逐 Judge 指标和原始票下钻；`evaluatorRelease.ts` 在 Active 前复算并冻结 Judge 集合与策略；旧单 Judge 历史兼容 | 核心单测覆盖投票、平票、失败隔离、非法策略、证据篡改、精确请求、全局并发和发布门禁；`multi-judge-calibration.spec.ts` 以 Mock 验证 `2×3=6`、取消零调用、真值隔离、刷新持久化和三处 Axe，既有单 Judge 回归通过；本地与独立干净环境的 146 单测、2 压测、20 路由构建、全量 28 E2E 及 workflow run `33298736070` 两道 CI 全部通过 | 页面可选择多个 Judge、预览 Case×Judge 费用、确认后执行，并下钻原始票、逐 Judge 指标和仲裁结果；通过视觉验收 | Judge 校准 | [#32](https://github.com/boyuling-123/AI-API-workspace/pull/32)，[#33](https://github.com/boyuling-123/AI-API-workspace/pull/33) |
...
| POOL-001 | 建立模型、算法、Judge 统一资源池 | 已验证 | `resourceCatalog.ts` 从唯一 `TargetConfig` 确定性投影模型/算法类型与被测目标/Judge 角色；`ResourcePoolPanel.tsx` 提供统一目录，`WorkspaceBody.tsx` 的 Judge 候选复用同一目录 | `resourceCatalog.test.ts` 覆盖旧项目推断、角色资格、去重和输入不变；`resource-pool.spec.ts` 覆盖真实页面目录、Judge 筛选、编辑持久化、零调用、WCAG 与 390px；本地、独立干净环境及 workflow run `33475468716` 两道 CI 均通过 | 资源角色和能力由同一实体表达，目录无重复状态，并通过独立干净环境与 GitHub CI | 资源池 | [PR #43](https://github.com/boyuling-123/AI-API-workspace/pull/43) |
| POOL-002 | 记录文本、图片、文生图、编辑、视频和业务算法能力 | 已验证 | `ResourceCapability` 定义六类能力；预置和自定义资源可显式维护，旧资源由 `resourceCatalog.ts` 向后推断；页面支持能力展示和筛选 | `resourceCatalog.test.ts` 覆盖显式元数据规范化与旧项目推断；`resource-pool.spec.ts` 验证文生图筛选和业务算法编辑持久化；本地、独立干净环境及 workflow run `33475468716` 两道 CI 均通过 | 六类能力可维护、筛选、兼容旧项目，并通过独立干净环境与 GitHub CI | 资源池 | [PR #43](https://github.com/boyuling-123/AI-API-workspace/pull/43) |
| POOL-003 | 记录输入输出模态和参数范围 | 已验证 | `resourceCatalog.ts` 规范 text/image/number/boolean 输入模态及 text/image 输出模态；`ParamDef.min/max` 与 `ApiConfigForm.tsx` 支持类型化默认值、数值范围编辑和保存前校验，资源卡脱敏展示必填、默认值与范围 | `resourceCatalog.test.ts` 覆盖模态、默认值脱敏/截断、`1–8` 范围、非有限值隔离和源对象不变；`resource-pool.spec.ts` 验证最小值/默认值越界阻断、范围 `1–8` 更新为 `2–8`、默认值更新为 `4`、刷新投影与零调用；本地、独立干净环境及 workflow run `33475468716` 两道 CI 均通过 | 输入输出模态、必填、默认值和合法数值范围可校验，并通过独立干净环境与 GitHub CI | 资源池 | [PR #43](https://github.com/boyuling-123/AI-API-workspace/pull/43) |
| POOL-004 | 记录接口来源、版本、别名和有效状态 | 已验证 | `TargetConfig` 保存来源、版本、稳定别名、状态与最近测试时间；`resourceIdentity.ts` 校验全局身份并以调用指纹使过期测试状态失效；`resourceCatalog.ts` 与 `ResourcePoolPanel.tsx` 提供查询、筛选、告警和明确的非实时语义 | `resourceIdentity.test.ts` 与 `resourceCatalog.test.ts` 覆盖规范化、冲突、脏导入、指纹边界和组合查询；`resource-identity.spec.ts` 覆盖真实页面编辑、状态失效/恢复、仅一次显式 Mock 测试、WCAG 与 390px；本地、独立干净环境及 workflow run `33588301577` 两道 CI 全部通过 | 来源、版本、别名和最近一次明确连通性测试状态均可查询；调用配置变化后旧状态失效，且不会后台探测或自动调用模型 | 资源池 | [PR #44](https://github.com/boyuling-123/AI-API-workspace/pull/44) |
...
| REPORT-001 | 输出综合排行榜和单维度排行榜 | 已验证 | `evaluationLeaderboard.ts` 仅从历史原始分数计算确定性综合榜/单维度榜，`EvaluationLeaderboard.tsx` 展示正式名次、综合分、覆盖率、否决次数与维度均分 | 5 项真实源码单测覆盖权重归一、缺失值不补零、并列排名与旧记录；`evaluation-leaderboard.spec.ts` 验证综合榜、单维度反转、零新增调用、刷新和 WCAG；本地与独立干净环境的 quality、全量 32 项 Playwright 及 workflow run `33300809061` 两道 CI 均通过 | 排名口径可解释并可下钻原始分数 | 报告复核 | [PR #37](https://github.com/boyuling-123/AI-API-workspace/pull/37) |
| REPORT-002 | 勾选关注维度后动态重新排名 | 已验证 | 排行榜支持勾选任意关注维度、仅看单维度和恢复全选；平台按所选维度的原权重重新归一，计算过程不写回 `EvaluationRecord` | 单测验证单维度重排前后原记录字节级不变；Mock E2E 验证准确性单维度使名次反转且 API 调用数不增加，390px 页面无横向溢出；workflow run `33300809061` 两道 CI 通过 | 维度选择即时更新排名且不篡改原始结果 | 报告复核 | [PR #37](https://github.com/boyuling-123/AI-API-workspace/pull/37) |
| REPORT-003 | 查看模型、Case、维度评分和理由 | 已验证 | `EvalHistoryPanel.tsx` 保留输入、输出、模型、各维度分数、hover 理由、策略和点评明细；排行榜提供原始 Case 锚点 | `evaluation-leaderboard.spec.ts` 从排行榜下钻并验证 Case 输入和原始维度理由；本地、独立干净环境与 workflow run `33300809061` 的全量用户路径均通过 | 任一评分均能下钻到输入、输出、维度和理由 | 报告复核 | [PR #37](https://github.com/boyuling-123/AI-API-workspace/pull/37) |
| REPORT-004 | 查看 Judge 引用证据 | 已验证 | `evaluateService.ts` 要求每个目标的每个维度返回 1–3 条结构化文字或图片证据，服务端核验来源、目标、原文与图片序号并自行计算引用位置；`EvaluationEvidenceList.tsx` 在即时结果与历史详情展示引用，`excel.ts` 导出独立证据列；旧记录明确显示未保存结构化证据 | 12 项真实源码单测覆盖精确位置、标准答案、输入/输出图片、客户端组包、未知目标、伪造原文、缺失与重复证据、零调用前置阻断及 Excel；Mock E2E 覆盖即时展开、历史持久化、xlsx 回读、390px、WCAG 和零新增调用；本地、独立干净环境及 workflow run `33457148220` 的两道 GitHub CI 均通过 | 每条结论包含可定位到输入或输出的证据 | 报告复核 | [PR #41](https://github.com/boyuling-123/AI-API-workspace/pull/41) |
| REPORT-005 | 筛选低分、分歧、高风险和失败 Case | 已验证 | `evaluationCaseFilter.ts` 以历史加权分、否决结果和来源运行状态确定性识别四类 Case；历史详情支持可调阈值、任一/全部组合、清除、依据标签和筛选子集 Excel，校准复核队列继续覆盖 Judge 专项风险 | 5 项真实源码单测覆盖四类隔离、组合语义、阈值、旧记录补算、缺失值与稳定导出；Mock E2E 验证 5 Case、xlsx 回读、空态、刷新零调用、390px 和 WCAG；本地、独立环境及 workflow run `33302603833` 两道 CI 均通过 | 四类筛选可组合、清除并导出 | 报告复核 | [PR #38](https://github.com/boyuling-123/AI-API-workspace/pull/38) |
| REPORT-006 | 人工修改评分、标记 Bad Case 和补充意见 | 已验证 | `evaluationReview.ts` 为任意评价目标追加不可变人工复核事件，保留 AI 原始维度分、加权分和否决状态；详情显示人工有效分与 AI 原分、Bad Case、修改人、理由和历史版本，排行榜继续明确使用 AI 原分；校准 Case 仍由专项复核队列处理 | 8 项真实源码单测覆盖追加版本、原分不变、同毫秒顺序、旧分精度、策略重算、脱敏、非法输入和篡改隔离；Mock E2E 覆盖两轮改分、Bad Case、必填拦截、刷新持久化、排行榜口径、390px、WCAG 和零新增调用；本地、独立环境及 workflow run `33303845225` 两道 CI 均通过 | 人工覆盖保留原 AI 分数、修改人和理由 | 报告复核 | [PR #39](https://github.com/boyuling-123/AI-API-workspace/pull/39) |
| REPORT-007 | 输出含原始结果、配置与版本的 HTML 报告 | 已验证 | `evaluationHtmlReport.ts` 从已保存的 Task、EvaluationRecord、有效人工复核和已校验 Evaluator 版本生成单文件离线报告，包含 AI 原分榜、Rubric、Prompt、原始输出、Judge 证据、机器可读快照和可复算完整性指纹；`EvalHistoryPanel.tsx` 提供独立下载入口 | 7 项真实源码单测覆盖完整快照、指纹、脱敏、注入、损坏隔离、错配与循环数据；Mock E2E 实际下载并离线打开 HTML，验证版本、data 图片、零外部请求、零新增模型调用、WCAG 与 390px；本地、独立干净环境及 workflow run `33472850294` 的两道 GitHub CI 均通过 | 离线 HTML 可复现结果并包含完整版本快照 | 报告复核 | [PR #42](https://github.com/boyuling-123/AI-API-workspace/pull/42) |
...
- POOL-001～003 已由 PR #43 完成统一目录投影、六类能力、输入输出模态、类型化默认值和数值范围维护；本地、独立干净环境、197 项单测、2 项压力测试、20 路由构建、全量 37 项 Mock Playwright/WCAG、视觉检查与 workflow run `33475468716` 两道 CI 全部通过，状态为“已验证”。
