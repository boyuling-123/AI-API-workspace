...
| DOC-003 | 准确说明维度生成上下文与 OpenJudge 状态 | 已实现 | `EvaluationPanel.tsx` 明示 OpenJudge 与 Iterative Rubrics Generator 未接入；`dimensionGeneration.ts`、`dimensionHumanFeedback.ts` 与 `genDimensionsService.ts` 使用目标、场景、任务类型、内部预设、有界样本、硬规则、显式 Bad Case 和人工反馈 | `dimensionGeneration.test.ts` 覆盖真实请求、Prompt、边界与脱敏；`dimension-generation.spec.ts`、`dimension-rules.spec.ts` 与 `dimension-human-feedback.spec.ts` 覆盖页面口径、Mock 精确请求与零自动评价；PR #18、#19 已通过 CI，PR 04C 本地全门禁通过 | 文档、页面、Schema 与真实模型输入一致；准确区分当前人工反馈上下文、未接入的 OpenJudge 和未实现的完整 Rubrics Generator | PR 01 / 04A / 04B / 04C | [#9](https://github.com/boyuling-123/AI-API-workspace/pull/9)；[#18](https://github.com/boyuling-123/AI-API-workspace/pull/18)；[#19](https://github.com/boyuling-123/AI-API-workspace/pull/19)；PR 04C 待创建 |
...
| DIM-004 | 维度生成支持标准答案、硬规则、Bad Case 和人工结果 | 已实现 | `dimensionGeneration.ts`、`dimensionHumanFeedback.ts`、`EvaluationPanel.tsx` 与 `genDimensionsService.ts` 已接入标准答案、最多 20 条硬规则、显式 Bad Case、`0–10` 人工评分和完整 `1..N` 偏好排序，并执行双层脱敏 | `dimensionHumanFeedback.test.ts`、`dimensionGeneration.test.ts` 与 `genDimensionsRoute.test.ts` 覆盖反馈 Schema、规范化、Prompt 和非法请求零模型调用；`dimension-rules.spec.ts` 与 `dimension-human-feedback.spec.ts` 以 Mock 覆盖阻断、精确目标绑定、零自动评价与 WCAG；本地及独立干净工作树的 72 单测、2 压测、17 E2E 均通过 | 标准答案、硬规则、Bad Case、人工评分和排序全部进入生成 Schema，并通过独立干净环境与 GitHub CI 证明边界和真实用户路径 | 维度 Skill | [#19](https://github.com/boyuling-123/AI-API-workspace/pull/19)；PR 04C 待创建 |
...
- DIM-002 与 DIM-003 已具备结构化代码、异常路径、真实源码测试、Mock 精确请求、视觉证据、独立干净环境和 PR #18 GitHub CI Trace，状态为“已验证”。DOC-003 已校准到当前人工反馈上下文，同时明确 OpenJudge 与 Iterative Rubrics Generator 未接入；PR 04C CI 完成前暂为“已实现”。
- DIM-004 已通过 PR #19 补齐标准答案、硬规则和 Bad Case，并在 PR 04C 本地补齐人工评分与偏好排序；双层脱敏、异常阻断、Mock 精确请求、视觉验收、本地与独立干净环境全门禁均通过，状态为“已实现”，待 GitHub CI 后升级为“已验证”。
