...
| TASK-002 | 配置并发数和 QPS | 已实现 | `RunPanel.tsx` 配置并发/QPS；`rateLimiter.ts` 平滑限制所有真实请求启动；`runService.ts` 让首次与重试共享限速器 | `rateLimiter.test.ts`；`runService.test.ts` 以假时钟验证 2 QPS 的启动时间为 0/500/1000ms；38 项单测、2 项压力测试、10 项 E2E 本地通过 | 并发和 QPS 均可配置并被调度器严格执行 | 任务控制 | PR 03B（本分支） |
...
| TASK-006 | 单任务超时和有限自动重试 | 已实现 | `Task.runPolicy` 固化超时/重试快照；`runService.ts` 每次尝试独立 Abort 并指数退避；`RunPanel.tsx` 上限配置；续跑沿用原策略 | `runService.test.ts` 覆盖真实 Abort、429 后成功、503 严格封顶和非重试错误；`run-controls.spec.ts` 覆盖策略持久化 | 超时与重试策略按目标配置并有次数上限 | 任务控制 | PR 03B（本分支） |
| TASK-007 | 超限后记录明确失败类型且不无限重试 | 部分实现 | `runError.ts` 八类错误；`ResultItem` 保存类型/次数/HTTP；历史结果可筛选并导出；失败项定向重跑待 TASK-005 | route/adapter/runService 真实源码单测；E2E 验证 401 配置重试 3 次仍只调用 1 次并可按鉴权失败筛选 | 超时、限流、鉴权、解析等错误可筛选和重跑 | 任务控制 | PR 03B（本分支） |
...
- TASK-002 与 TASK-006 已完成本地实现和自动化证据，等待 PR 03B GitHub CI 与最终 Trace 后再升级为“已验证”。
- TASK-007 已完成错误分类、有限重试、筛选和导出；失败项定向重跑依赖 TASK-005，因此继续保持“部分实现”。
