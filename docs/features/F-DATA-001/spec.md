...
- 当前状态：已实现，本地完整验收通过，远端 CI 待创建 PR 后执行。
...
- 测试：`tests/unit/localArchive.test.ts`（11 项）、`tests/e2e/local-archive.spec.ts`（3 项）；全量 220 unit、2 stress、43 E2E 通过，截图见 [证据](../../evidence/pr-local-history-demo/README.md)。PR 待创建；远端 CI 未通过前不标记已验证。
