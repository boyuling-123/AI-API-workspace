...
## 当前入口：v6.0 AI-Native 底座迁移

用户提供的新 v6.0 PRD 已替代 2026-09-07 的临时“暂不新增后端存储”范围。当前根目录 Next.js 应用冻结为 v5 迁移源；新系统以 EvalScope 固定提交 `c31f0f946c09bc087f856a017637ac157b5459a5` 为唯一代码基座，文件真相源与 SQLite 可重建索引重新进入 P0。

- V6-00：正在完成 PRD 审查、固定上游来源、领域对象、Action、存储与分阶段验收契约。
- V6-01：下一 Ready，建立可追溯 EvalScope Fork/工作树，完成 Python 3.12 无模型服务基线和依赖安全处置。
- V6-02：在 V6-01 后执行白底商品图合成试跑；通过前不批量迁移旧页面。
- 全部 v6 文档和状态口径见 [v6.0 重构基线](../v6/README.md)。
- 旧 75 项矩阵继续作为迁移清单，不与 v6 底座能力核验表合并计算完成率。

## 历史入口：2026-09-07 夜间源码复用开发
